...
var encodeAddressToBech32 = function encodeAddressToBech32(hexAddr) {
  var prefix = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "okexchain";
...
var getAddressFromPubKey = function getAddressFromPubKey(publicKey, prefix) {
  publicKey = publicKey.slice(0, 2) === '0x' ? publicKey.slice(2) : publicKey;

  var publicKey1 = _buffer.Buffer.from(publicKey, 'hex');

  publicKey = _buffer.Buffer.from(_secp256k["default"].publicKeyConvert(new Uint8Array(publicKey1), false)).slice(1);
  var hash = (0, _keccak["default"])('keccak256').update(publicKey).digest();
...
var sign = function sign(msgHex, privateKey) {
  var msg = _buffer.Buffer.from(msgHex, "hex");

  var msgHashHex = (0, _keccak["default"])('keccak256').update(msg).digest();

  var msgHash = _buffer.Buffer.from(msgHashHex, "hex");

  var signature = _tinySecp256k["default"].sign(msgHash, _buffer.Buffer.from(privateKey, "hex")); // enc ignored if buffer
...
var validateSig = function validateSig(sigHex, msgHex, pubKeyHex) {
  ...
  return _tinySecp256k["default"].verify(msgHash, publicKey, _buffer.Buffer.from(sigHex, "hex"));
};
/**
